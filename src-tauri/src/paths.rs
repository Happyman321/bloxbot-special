...
/// Returns the bundled VS Code MCP server script.
pub fn bundled_vscode_mcp_server_path() -> Result<PathBuf, String> {
    let sidecar = sidecar_dir()?;
    ...
    let dev_path = sidecar
        .parent()
        .and_then(|p| p.parent())
        .map(|p| p.join("resources").join("vscode-mcp").join("server.js"))
        .unwrap_or_default();
    ...
    Err(format!(
        "Bundled VS Code MCP server not found. Checked:\n  {}\n  {}",
        prod_path.display(),
        dev_path.display()
    ))
}

/// Returns the bundled Roblox DevForum knowledge MCP server script.
pub fn bundled_devforum_mcp_server_path() -> Result<PathBuf, String> {
    let sidecar = sidecar_dir()?;
    ...
    let dev_path = sidecar
        .parent()
        .and_then(|p| p.parent())
        .map(|p| p.join("resources").join("devforum-mcp").join("server.mjs"))
        .unwrap_or_default();
    ...
    Err(format!(
        "Bundled Roblox DevForum MCP server not found. Checked:\n  {}\n  {}",
        prod_path.display(),
        dev_path.display()
    ))
}
...
mod tests {
    ...
    #[test]
    fn bundled_devforum_mcp_server_path_does_not_panic() {
        let result = bundled_devforum_mcp_server_path();
        match result {
            Ok(path) => assert!(path.exists()),
            Err(msg) => assert!(
